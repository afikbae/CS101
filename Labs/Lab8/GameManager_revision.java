/*
    ...
    Lab 8 - Game Manager class revision
    @date 08.12.2022
*/
 ...
 */
public class GameManager_revision 
{
    ...
    private static final int INITIAL_PLAYER_HEALTH = 20;
    private static final int MONSTER_DAMAGE_TO_PLAYER = INITIAL_PLAYER_HEALTH;
    private static final int BUG_DAMAGE_TO_PLAYER = 10;
    ...
    { 
        return GameManager_revision.NO_OF_COLUMNS;
    }
    ...
    { 
        return GameManager_revision.NO_OF_ROWS;
    }
    ...
    public void setupGame() 
    {
        ...
        this.bug.positionAt( GameManager_revision.NO_OF_COLUMNS / 2, GameManager_revision.NO_OF_ROWS / 2);
        this.monster = new Enemy('M');
        this.monster.positionAt(GameManager_revision.NO_OF_COLUMNS - 1, GameManager_revision.NO_OF_ROWS - 1);
    }
    ...
    {
        displayRowSeparator();
        for (int i = 0; i < GameManager_revision.NO_OF_ROWS; i++)
        {
            System.out.print("|");
            for (int j = 0; j < GameManager_revision.NO_OF_COLUMNS; j++)
            {
                String square = "";
                if (GameManager_revision.doCoordsMatch(this.player.getX(), this.player.getY(), j, i))
                {
                    square += this.player.getIcon();
                }
                if (GameManager_revision.doCoordsMatch(this.bug.getX(), this.bug.getY(), j, i))
                {
                    square += this.bug.getIcon();
                }
                if (GameManager_revision.doCoordsMatch(this.monster.getX(), this.monster.getY(), j, i))
                {
        ...
        String healthBar = "";
        for (int i = 0; i < this.player.getHealth(); i++) healthBar += "*";
    ...
    {
        if (GameManager_revision.doCoordsMatch(this.player.getX(), this.player.getY(), this.monster.getX(), this.monster.getY()))
        {
            this.player.loseHeath(MONSTER_DAMAGE_TO_PLAYER);
        }
        if (GameManager_revision.doCoordsMatch(this.player.getX(), this.player.getY(), this.bug.getX(), this.bug.getY()))
        {
    ...
    {
        return GameManager_revision.doCoordsMatch(this.player.getX(), this.player.getY(), GameManager_revision.TARGET_X, GameManager_revision.TARGET_Y);
    }
    ...
    {
        GameManager_revision gm = new GameManager_revision();
        ...
        while (!gm.isGameOver() && dirChar != 'q') {            
            ...
            {
                player.changeDirection(GameManager_revision.charToDirection(dirChar));
            }
